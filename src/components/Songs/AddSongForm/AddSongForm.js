...
export default function AddSongForm(props) {
  ...
  const onSubmit = () => {
    if (!formData.name || !formData.album) {
      toast.warning(
        "El nombre de la canción y el album al que pertenece son obligatorios."
      );
    } else if (!file) {
      ...
      uploadSong(fileName)
        .then(() => {
          db.collection("songs")
            ...
            .catch(() => {
              toast.error("Error al subir la canción111");
              setIsLoading(false);
            });
        })
        .catch(() => {
          toast.error("Error al subir la canción222.");
          setIsLoading(false);
        });
    }
  };
    ...
    <Form className="add-song-form" onSubmit={onSubmit}>
      ...
      <Form.Field>
        <div className="song-upload" {...getRootProps()}>
          <input {...getInputProps()} />
          <Icon name="cloud upload" className={file && "load"}></Icon>
          <div>
            <p>
              {" "}
